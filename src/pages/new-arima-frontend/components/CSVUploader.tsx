// disable typescript error for this entire file
...
const CSVUploader: React.FC<CSVUploaderProps> = ({
  ...
  const handleColumnChange = async (changedFields: any, allFields: any) => {
    ...
    const dateColumn = allFields.find(
      (f: any) => f.name[0] === "dateColumn"
    )?.value;
    const valueColumn = allFields.find(
      (f: any) => f.name[0] === "valueColumn"
    )?.value;

    if (dateColumn && valueColumn && fileInfo.rawData) {
      try {
        const needsAggregation = shouldAggregateData(
          fileInfo.rawData,
          dateColumn
        );

        if (needsAggregation) {
          const preprocessingResult = aggregateDataByDay(
            fileInfo.rawData,
            dateColumn,
            valueColumn
          );
          ...
          const processedFile = createAggregatedCSVFile(
            preprocessingResult.aggregatedData,
  ...
  const handleSubmit = async (values: any) => {
    ...
    const config: TrainingConfig = {
      ...
      valueColumn: processingInfo?.needsAggregation
        ? "value"
        : values.valueColumn,
      ...
      <Space direction="vertical" style={{ width: "100%" }} size="large">
        ...
        <Dragger {...uploadProps} style={{ padding: "20px" }}>
          ...
          <p className="ant-upload-hint" style={{ color: "#999" }}>
            Support for pizza sales data in CSV format. File should contain date
            and sales columns.
          </p>
        </Dragger>
        ...
        {processingInfo?.needsAggregation &&
          processingInfo.preprocessingResult && (
            <Alert
              message="🔄 Data Preprocessing Applied"
              description={
                <div>
                  <p>
                    Your dataset contains multiple transactions per day and has
                    been automatically aggregated:
                  </p>
                  <ul style={{ margin: "8px 0", paddingLeft: "20px" }}>
                    <li>
                      Original:{" "}
                      {processingInfo.preprocessingResult.originalRowCount.toLocaleString()}{" "}
                      individual transactions
                    </li>
                    <li>
                      Aggregated:{" "}
                      {processingInfo.preprocessingResult.aggregatedRowCount}{" "}
                      daily totals
                    </li>
                    <li>
                      Date Range:{" "}
                      {processingInfo.preprocessingResult.dateRange.start} to{" "}
                      {processingInfo.preprocessingResult.dateRange.end}
                    </li>
                  </ul>
                  <Text type="secondary">
                    This optimization reduces training time from hours to
                    minutes while maintaining forecast accuracy.
                  </Text>
                </div>
              }
              type="info"
              ...
              <Row gutter={16}>
                <Col span={12}>
                  <Form.Item
                    ...
                    name="dateColumn"
                    rules={[
                      { required: true, message: "Please select date column" },
                    ]}
                  >
                    <Select placeholder="Select date column">
                      {fileInfo.columns
                        .filter((col) =>
                          /order.*date|date.*order|order_date|orderdate/i.test(
                            col
                          )
                        )
                        .map((col) => (
                ...
                <Col span={12}>
                  <Form.Item
                    ...
                    name="valueColumn"
                    rules={[
                      { required: true, message: "Please select value column" },
                    ]}
                  >
                    <Select placeholder="Select value column to forecast">
                      {fileInfo.columns
                        .filter((col) =>
                          /quantity|total.*price|price.*total|total_price|totalprice/i.test(
                            col
                          )
                        )
                        .map((col) => (
              ...
              <Row gutter={16}>
                <Col span={12}>
                  <Form.Item
                    ...
                    name="testSize"
                    rules={[
                      { required: true, message: "Please select test size" },
                    ]}
                  >
                    <Select>
                      <Option value={0.1}>
                        10% (90% training, 10% testing)
                      </Option>
                      <Option value={0.2}>
                        20% (80% training, 20% testing)
                      </Option>
                      <Option value={0.3}>
                        30% (70% training, 30% testing)
                      </Option>
                    </Select>
                  </Form.Item>
                </Col>
                <Col span={12}>
                  <Form.Item
                    ...
                    rules={[
                      {
                        required: true,
                        message: "Please select forecast horizon",
                      },
                    ]}
                  >
                    <Select>
